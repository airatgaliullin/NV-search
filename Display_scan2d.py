...
class DisplayScan(m2.M2Analysis):
    ...
    def plot_data(self,title,save=True, **kw):
        ...
        #this
        #lt.colorbar(colors,ax = ax, fraction=0.046, pad=0.04)
    ...
    def split_image(self,title,x_scan,y_scan,save=True, **kw):
        x_ind=[]
        y_ind=[]
        
        search_range=kw.pop('search_range', 2)
        # area_size=kw.pop('area_size',4)
        # x_o=kw.pop('x_o',0)
        # y_o=kw.pop('y_o',0)
        # x_start=x_o-area_size/2
        # x_stop=x_o+area_size/2
        # y_start=y_o-area_size/2
        # y_stop=y_o+area_size/2
        ...
        lx=np.arange(x_start,x_stop,search_range)
        lx=np.append(lx,x_stop)
        ly=np.arange(y_start,y_stop,search_range)
        ly=np.append(ly, y_stop)
        ...
        for i in lx:
            x_ind.append(np.abs(self.xvalues-i).argmin())

        for i in ly:
            y_ind.append(np.abs(self.yvalues-i).argmin())





        for i in xrange(0,(len(x_ind)-1)):
            crop_x=self.xvalues[x_ind[i]:x_ind[i+1]+1]
            x_c=np.mean(crop_x)

            for j in xrange(0,(len(y_ind)-1)):
                
                crop_y=self.yvalues[y_ind[j]:y_ind[j+1]+1]
                y_c=np.mean(crop_y)
                ...
                countrates_local=self.countrates[y_ind[j]:y_ind[j+1]+1,x_ind[i]:x_ind[i+1]+1]
                vmax=np.amax(countrates_local)

                vmin=np.amin(countrates_local)
                ...
                use_save_location = kw.pop('use_save_location',False)
                save_location = kw.pop('save_location',None)
                colormap=kw.pop('colormap','gist_earth')
                grid = kw.pop('grid',False)
                fig = plt.figure(**kw)
                ax = fig.add_subplot(1,1,1)
                colors=ax.imshow(countrates_local, cmap=colormap, interpolation='none',
                         vmin=vmin,vmax=vmax,
                         extent=[np.amin(crop_x),np.amax(crop_x),np.amin(crop_y),np.amax(crop_y)], origin='lower')
                

                #ax.set_xlim([np.amin(crop_x),np.amax(crop_x)])
                #ax.set_ylim([np.amin(crop_y),np.amax(crop_y)])
                #fig.colorbar(colors,ax=ax)
                ax.axis('off')
                #plt.show()
                if save:
                    try:
                        fig.savefig(
                            os.path.join(self.folder,' x_c='+str(math.modf(x_c)[1])[:-2]+' y_c='+str(math.modf(y_c)[1])[:-2]+' .png'))
                        if use_save_location and (save_location!=None):
                            min_x = min(self.xvalues)
                            min_y = min(self.yvalues)
                            depth = self.folder[-4:]
        ...
        vmax = kw.pop('vmax', np.amax(self.countrates))
        vmin = kw.pop('vmin', np.amin(self.countrates))
        #ns = [np.argmin(abs(self.xvalues-k)) for k in np.arange(0,max(self.xvalues),2)]
        # ms = [np.argmin(abs(self.yvalues-k)) for k in np.arange(0,max(self.yvalues),2)]

        # for i,n in enumerate(ns):
        #     for j,m in ms:
        #         self.countrates_nm = self.countrates[n:n[i+1],m:m[j+1] ]   #crop z data
        #         #plot this
